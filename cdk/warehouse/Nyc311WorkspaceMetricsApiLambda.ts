 ...
 * workspace's metric tiles, read from the latest `wbr` job result. Its own
 * Lambda so its `Duration` metric (Lambda Health) is this endpoint's
 * latency alone. Entry point is
 * `backend/controller/web-api/getWorkspaceMetricsController.ts`. Same
 * read-only grants as `Nyc311JobResultApiLambda`.
 */
