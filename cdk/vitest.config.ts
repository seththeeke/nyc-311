...
export default defineConfig({
  test: {
    ...
    coverage: {
       ...
       * the recurring Operational-Loop run (and CI's sharded merge step)
       * emitted ~38 zero-signal rows here. Coverage math, the perFile 90%
       * gate, and the html/json reporters consumed by CI and
       * scripts/rollup-coverage.js are untouched; a file below 100% still
       * shows, below 90% still fails.
       */
