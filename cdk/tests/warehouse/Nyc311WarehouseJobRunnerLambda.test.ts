...
describe("Nyc311WarehouseJobRunnerLambda", () => {
  it("bundles runWarehouseJobController on Node 22 with warehouse config as env (Leg 8 — no more WAREHOUSE_JOBS manifest)", () => {
  ...
  it("does not carry the removed WAREHOUSE_JOBS / ANALYTICS_ROLLUPS_TABLE_NAME / SAMPLE_JOB_SQL env vars", () => {
    ...
    expect(Object.keys(vars)).not.toContain("WAREHOUSE_JOBS");
  ...
  it("its S3 grants are read on data/*+job-definitions/*, read+write on job-results/* and athena-results/*, delete on athena-results/*", () => {
    ...
    /* the s3:PutObject statement's resources name job-results/ and athena-results/, not data/ or job-definitions/ */
