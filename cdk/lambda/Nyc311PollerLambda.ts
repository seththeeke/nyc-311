 ...
 * `backend/controller/ingestion/nyc311PollerController.ts`
 * (`1-data-ingestion.md`, `CLAUDE.md` §5.2's "ingestion" controller group).
 * Bundled by esbuild via `NodejsFunction` at synth/deploy time; `backend/`
 * itself produces no deployable artifact of its own.
 *
 * Owns its own explicit log group (so `MetricFilter`s below have a handle
 * to attach to) and the least-privilege grant against its `RequestsTable` —
 * exactly the three actions `RequestDao` actually calls
 * (`GetItem`/`PutItem`/`Query`), nothing broader.
 */
