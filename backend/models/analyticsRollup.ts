...
/*
 * One pre-aggregated result row in the AnalyticsRollups table
 * (`7-data-warehousing.md` §8) — an Athena job's output, copied into
 * DynamoDB so the dashboard reads it like every other API. `metric_view`
 * is the partition key, `rollup_key` (`<run_date>#<dimension>`) the sort
 * key, so one view holds a daily time series across dimensions.
 */
