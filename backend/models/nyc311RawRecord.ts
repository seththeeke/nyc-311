...
/*
 * One row from the NYC 311 SODA API, before it becomes a Request. Per
 * 1-data-ingestion.md §4, only unique_key/created_date are required; shape
 * varies wildly by complaint_type, so the rest is optional. Field list is
 * from a real 1,387-record pull, not guaranteed-exhaustive, hence
 * .passthrough(). Location fields match data-model.md#location for a
 * future LocationResolver.
 */
