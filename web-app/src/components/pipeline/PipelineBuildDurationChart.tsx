...
export function PipelineBuildDurationChart({ executions }: PipelineBuildDurationChartProps): ReactElement {
  ...
  return (
    <div className="rounded-lg border border-line bg-panel p-4">
      ...
      {completed.length === 0 ? (
        <p className="text-sm text-fg-subtle">No completed builds in the current history yet.</p>
      ) : (
        <>
          <p className="text-xs text-fg-subtle">Last {completed.length} completed builds</p>
            ...
            <div className="flex items-end gap-2" style={{ height: CHART_HEIGHT_PX }}>
              {completed.map((execution) => (
                ...
                >
                  ...
                  <span
                    aria-hidden="true"
                    className="pointer-events-none absolute bottom-full left-1/2 z-10 mb-1 w-max max-w-48 -translate-x-1/2 rounded-md bg-tooltip px-2.5 py-1.5 text-[10px] text-tooltip-fg opacity-0 shadow-lg transition-opacity group-hover:opacity-100 group-focus-within:opacity-100"
                  >
                    <span className="block font-medium">{formatDurationSeconds(execution.buildDurationSeconds)}</span>
                    <span className="block text-fg-muted">{formatRelativeTime(execution.startTime)}</span>
